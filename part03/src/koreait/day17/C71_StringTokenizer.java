...
// token : 표식(최소 단위로 분류)
	...
	public static void main(String[] args) {
		...
		while (stk.hasMoreTokens()) {		// 다음에 읽어올 토큰이 있으면 while문 반복
			System.out.println(stk.nextToken());
		}
		System.out.println();
		
		System.out.println("3. 여러가지 구분 기호 사용하기");
		temp = "모모,90 88()79";				// empty는 만들지 않는다. (split과의 차이점) split과의 차이점 ,,의 경우 split은 ,하나를 문자열으로 인식하지만 Tokenizer는 그렇지 않다.

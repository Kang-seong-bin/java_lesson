...
//		read(mywords,"D:\\KSB\\나의영한사전.txt");			//파일에서 기존 내용 읽어오고 그 내용을 mywords 리스트에 저장
		try {
			System.out.println("파일을 읽어올까요 ? (y or n) >>> ");
			if(ssc.nextLine().equals("y"))
				//파일에서 기존 내용 읽어오고 그 내용을 mywords 리스트에 저장
			read(mywords,"D:\\KSB\\나의영한사전.txt");
			else
				System.out.println("파일을 새로 만듭니다.");
		} catch (FileNotFoundException e1) {
			System.out.println("파일 저장 중에 오류가 생겼습니다.-" + e1.getMessage());
		}

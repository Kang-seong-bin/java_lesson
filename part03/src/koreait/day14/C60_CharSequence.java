...
public class C60_CharSequence {
		...
		//CharSequence 인터페이스 구현체 : String, String buffer, StringBuilder...
	public static void main(String[] args) {
		
		String result = new String();			//result="test"; result = new String("test");
		System.out.println("초기 result : " + result);	//값은 ""안의 빈칸 이유로는 참조값을 가지고 있기 때문
		...
		StringBuffer sb = new StringBuffer();	//StringBuffer와 함께 많이 사용되는 클래스
		System.out.println("2. StringBuffer 타입 테스트");
		for(int i=2;i<6;i++) {
			//StringBuffer 객체의 문자열 내용이 바뀌어도 기존 객체에 추가되는 동작을 합니다.
			//반복되는 연결 연산이 많은 경우 String?x StringBuffer?o buffer가 효율적임
		...
		//		HashMap	    선언할 때 변수 선언 Map 인터페이스 타입으로 합시다.
		List<String> names = new ArrayList<>();
		names = new Vector<String>();
	
		Map<String, Integer> map = new HashMap<String, Integer>();	//객체 생성시 제너릭타입 생략가능

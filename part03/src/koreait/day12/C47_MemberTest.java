...
public class C47_MemberTest {
	...
	public static void main(String[] args) {
		...
		Member member = new Member("다현", 26);	//변수를 활용하여 list안에 추가하는 방법 아래줄까지 작성
		list.add(member);
		...
		System.out.println("2. list의 요소 중 참조 객체 age 필드의 필드 값이 25이하 인 것만 출력해보기.");
		int i;
		...
		//리스트 요소가 여러 개의 필드를 갖는 객체일 떄, 필드값 비교하는 equals 예제입니다.
		
		System.out.println("4. 참고");
		//			indexOf 메소드는 제너릭타입이 String, 기본형 래퍼클래스만 추가적인 기능 구현 없이 사용합니다.
		System.out.println("Member 객체로 indexOf 실행 : " + list.indexOf(new Member("나나", 20)));	
		//-1, 못찾음 (이유 : 참조값이 같은 것을 찾는 것인데 이것을 필드값 비교해서 찾도록 바꾸어 주는기능 구현이 필요합니다.)

		System.out.println("Member 객체로 indexOf 실행 : " + list.indexOf(member));	//1, indexOf와 for문을 활용하여  
		for(i=0;i<list.size();i++) {

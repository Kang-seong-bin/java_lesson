 ...
 */
	...
	static int num;
	static double point;
	static final int TEST =9999;		//static final 이 상수. 선언할 때 초기화 하지 않으면 오류
	
	
	public static void main(String[] args) {	//★★static 은 static 만 접근합니다 ★★
		int num = 10;		//main메소드에서만 사용 가능
		System.out.println("num=" + num);	//10
		//System.out.println("message=" + message);	//오류 : static 이 아닌 변수는 사용 불가
		methodB();
		//methodA();//오류 : static 이 아닌 메소드
	}
	
	
	public void methodA() {	//static 메소드가 아니므로 객체를 만들어서 사용하는 메소드
		int num=99;//methodA에서만 사용하는 변수로 중복된 변수명인 num이라도 서로의 영역에 영향이 없어 사용 가능
	...
	public static void methodB() {
		int num=77;//methodB에서만 사용하는 변수로 중복된 변수명인 num이라도 서로의 영역에 영향이 없어 사용 가능
		//지역변수와 전역변수 이름이 같은 경우. 지역변수를 가져옵니다.
		System.out.println("num=" + num);	//99
		System.out.println("num=" + C35_GlobalVar.num);		//Integer.BYTES 와 같은형태
		//전역변수로 선언된 num. static 일 때는 클래스 이름. 변수명이고 static이 아닐 때는 this.변수명
	}

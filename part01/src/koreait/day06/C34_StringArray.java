...
public class C34_StringArray {
	...
	public static void main(String[] args) {
		...
		String[] koreans = new String[5];	//배열 요소의 값이 기본형데이터 저장하는 것이 아니라 참조값을 저장하는 것이다.
		...
		for(int i=0; i<koreans.length;i++)
			if(koreans[i] != null)	//★★★null이 아닐 때만 실행하라는 문장 중요!
			System.out.println(koreans[i] + ":" + koreans[i].length());
		//참조하고 있는 객체가 없을 때 메소드/필드 참조하면 오류 (초기값인 null로 설정되어 있는 koreans)
											//			ㄴ koreans[3],koreans[4]은 null을 참조하고 있어서
	//String temp = null;						//					ㄴ오류를 없애기 위해 위의 if문을 활용
	//System.out.println(temp.length()); //오류
		...
		System.out.println("4. 문자열 배열에 비교하기");
		...
		//break 로 for 를 중단하지 않고 for를 모두 실행했을 때는 같은 문자열이 배열 engKey에 없을 때 입니다.
		//i=engkey.length
		System.out.println("찾은 단어 위치 : ");
		if(i==engKey.length)
		System.out.println("찾은 단어가 engKey 에 없습니다.!");
		else
